...
(function (root, factory) {
	...
	PickerState.prototype.getBatchSize = function(currentSize) {
		...
		return Math.max(3, this.settings.minBatchSize || 0, Math.min(this.settings.maxBatchSize || 5, Math.ceil(Math.sqrt(currentSize))));
	};
	...
	Picker.prototype.setState = function(state) {
		if (localStorage && JSON && this.options.localStorageKey) {
			try {
				state = JSON.parse(localStorage.getItem(this.options.localStorageKey));
			} catch (e) {
				return null;
